    ...
    fn query_via_syng_raw(
        ...
        Ok(intervals
            ...
            .filter_map(|iv| {
                ...
                let interval = coitrees::Interval::new(iv.start as i64, iv.end as i64, id);
    ...
    fn query_via_syng_filtered(
        ...
        Ok(chained
            ...
            .filter_map(|c| {
                ...
                let t_iv = coitrees::Interval::new(c.start as i64, c.end as i64, tid);
                ...
                let members: Vec<(String, Vec<u8>, syng_graph::Member)> = sequences
                    ...
                    .map(|(seq, meta)| {
                        let (fwd_start, fwd_end) = if meta.strand == '+' {
                            ...
                            (
                                (meta.total_length as i64 - meta.start - meta.size) as u64,
                                (meta.total_length as i64 - meta.start) as u64,
                            )
                        };
                        (
...
";
    ...
    fn syng_local_rebuild_uses_requested_syncmer_length() {
        ...
        let intervals = vec![coitrees::Interval::new(0, seq.len() as i64, seq_id)];
